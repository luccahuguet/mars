...
| runtime | Emoji and icon glyph fallback | Preserves emoji metadata for symbol-map font hits and maps packaged yzxterm prompt/status emoji ranges to scalable `Noto Emoji` while keeping private-use icon glyphs on `Symbols Nerd Font Mono` | Prompt emoji and common status glyphs use the packaged scalable emoji face instead of rough text fallback or downsampled bitmap color emoji, and renderer layout keeps emoji width handling intact. | Gives yzxterm a clean place to tune emoji/icon fallback quality without moving terminal-specific font logic into main Yazelix. | `rustfmt --check sugarloaf/src/font/mod.rs`; `git diff --check`; `nix-instantiate --parse pkgRio.nix`; manual fresh-window glyph screenshot evidence |
